...
def main(input_file):
    ...
    with open(input_file, 'r') as file:
        ...
        arrsize = len(lines)
        coordarr = np.zeros((arrsize, 2))
        for i, line in enumerate(lines):
            coordarr[i] = [float(coord) for coord in line.strip().split()]
            
        
    dist = np.array([val]*arrsize)
    obsarr = np.column_stack(((coordarr, dist)))
...
if __name__=="__main__":
    import sys
    if len(sys.argv) != 2:
        print(f"Usage: python script_name.py input_file")
        sys.exit(1)
